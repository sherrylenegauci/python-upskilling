...
def compare_records(azara_record, rui_record):
    ...
    """
    format_coordinate = convert_coordinate(azara_record[-1])

    return format_coordinate in rui_record
...
def clean_up(combined_record_group):
    ...
    for record in combined_record_group:
        clean_list = list(record)
        clean_list.pop(1)
        clean_tuple = tuple(clean_list)
        report += f"{clean_tuple}\n"
    return report
